...
```
## 8. setState数组不更新（setState使用Object.is()比较新旧值，对象比较的是引用地址）
错误写法：
```js
...
## 9. git commit 报错：pidtree\lib\pidtree.js    new Error: no matching pid found
...
```
## 11. 自己在本地新建了个项目，关联远程仓库时git push报错：The requested URL returned error: 403
  解决：https://blog.csdn.net/qq_40226073/article/details/119801341
  思路是生成仓库的token令牌，git绑定密码为这个令牌。
## 12. eslint检查报错:'xxx' is missing in props validation react/prop-types
这是因为组件的prop没写类型声明，加上类型声明就可以解决。
## 13. To many re-renders.React limit the number of renders to prevent an infinity loop.
